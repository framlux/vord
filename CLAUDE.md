...
**Server configuration settings (replica-safe cache):** admin-facing settings (`ServerConfigurationSettings`) are read through three layers: the per-process `ServerSettingsCache` (5-min TTL, in the `database` project), a shared Redis read-through in `ServerConfigurationService` (`config:{key}`), and Postgres as the source of truth. The Redis read-through repopulates from the **database** (`IServerSettingsCache.GetSettingFromDatabaseAsync`), never the local cache, so a just-invalidated key is not re-seeded stale. Every write path — REST `AdminHandler` and gRPC `FleetAdminService` — must, **after commit**: invalidate the local cache and call `ServerSettingsInvalidation.PublishAsync` (deletes the Redis key and publishes the integer key on the `config:invalidate` pub/sub channel). `ServerSettingsCache` subscribes to that channel and clears the named entry across replicas; pub/sub is best-effort with the TTL as the correctness backstop. Both write paths validate through the single `ServerSettingValidation` helper. Because of the subscription, the `database` project takes a `StackExchange.Redis` dependency and `ServerSettingsCache`'s constructor requires an `IConnectionMultiplexer`. Consumers that must react promptly to a flip (e.g. the `AllowUserSignup` signup kill-switch) read via `ServerConfigurationService`'s Redis read-through, not `ServerSettingsCache` directly.
