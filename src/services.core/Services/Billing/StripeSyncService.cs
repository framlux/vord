...
public sealed class StripeSyncService : BackgroundService
{
        ...
        string proPriceId, string teamPriceId, CancellationToken ct)
    {
        ...
        if (subscription.Tier != stripeTier.Value)
        {
            ...
            Framlux.FleetManagement.Services.Core.Handlers.IBillingWebhookHandler webhookHandler =
                scope.ServiceProvider.GetRequiredService<Framlux.FleetManagement.Services.Core.Handlers.IBillingWebhookHandler>();
