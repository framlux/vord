...
public sealed class InitialMigration : Migration
{
    ...
    public override void Up()
    {
        ...
        IfDatabase("SQLite").Create.Table(TableNames.MachineTelemetry)
            ...
            .WithColumn("RetentionClass").AsInt16().NotNullable()
        ...
        // MachineTelemetry is partitioned LIST("RetentionClass") -> per-class RANGE("ReceivedAt") so
        // each retention class owns its own daily partitions and is dropped on its own schedule. The
        // partition columns (RetentionClass, ReceivedAt) must be part of the primary key.
        IfDatabase("PostgreSQL").Execute.Sql("""
        ...
        // One LIST parent per retention class, each further partitioned by day on ReceivedAt. The
        // daily leaf partitions under each class parent are created by the migration bootstrap below
        // and maintained thereafter by PartitionManagementJob.
        IfDatabase("PostgreSQL").Execute.Sql("""
        ...
        // On Postgres every column of the partition key must be included in a unique index; the table
        // is now partitioned LIST("RetentionClass") -> RANGE("ReceivedAt"), so both partition columns
        // are part of the index. A re-delivery of the same event resolves to the same class and
        // ReceivedAt, so it still collides on this index.
        IfDatabase("PostgreSQL").Execute.Sql(
            @"CREATE UNIQUE INDEX ""IX_MachineTelemetry_SourceEventId""
              ON ""MachineTelemetry"" (""SourceEventId"", ""RetentionClass"", ""ReceivedAt"")
        ...
        // MachineTelemetry is composite (LIST by class -> RANGE by day), so its dailies are created
        // under each class parent; the other tables are simple daily range partitions.
        CreateInitialClassDailyPartitions("MachineTelemetry_Short");
        CreateInitialClassDailyPartitions("MachineTelemetry_Medium");
        CreateInitialClassDailyPartitions("MachineTelemetry_Long");
    ...
    /// <summary>
    /// Creates the bootstrap set of daily leaf partitions (today + 7 days ahead) plus a default
    /// partition under a single MachineTelemetry retention-class parent (e.g.
    /// <c>MachineTelemetry_Short</c>). The leaf names mirror the class-qualified names
    /// PartitionManagementJob emits (<c>{classParent}_{yyyyMMdd}</c>) so its idempotent create/drop
    /// finds and manages them. No-op on SQLite, which is not partitioned.
    /// </summary>
    /// <param name="classParentTable">The retention-class parent table the leaves belong to.</param>
    private void CreateInitialClassDailyPartitions(string classParentTable)
    {
        DateOnly today = DateOnly.FromDateTime(DateTime.UtcNow);
        ...
        for (int offset = 0; offset <= 7; offset++)
        {
            DateOnly day = today.AddDays(offset);
            DateOnly nextDay = day.AddDays(1);
            string partitionName = $"{classParentTable}_{day:yyyyMMdd}";

            IfDatabase("PostgreSQL").Execute.Sql(
                $@"CREATE TABLE ""{partitionName}"" PARTITION OF ""{classParentTable}""
                       FOR VALUES FROM ('{day:yyyy-MM-dd}') TO ('{nextDay:yyyy-MM-dd}')");
        }

        IfDatabase("PostgreSQL").Execute.Sql(
            $@"CREATE TABLE ""{classParentTable}_default"" PARTITION OF ""{classParentTable}"" DEFAULT");
    }
}
