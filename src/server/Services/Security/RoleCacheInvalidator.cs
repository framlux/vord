...
/// <see cref="CookiePrincipalValidator"/> refreshes them from the database on the next request.
/// </summary>
public sealed class RoleCacheInvalidator : IRoleCacheInvalidator
{
    ...
    public async Task InvalidateAsync(int userId, CancellationToken ct)
    {
        ...
        string cacheKey = $"{CookiePrincipalValidator.RoleCacheKeyPrefix}{userId}";
