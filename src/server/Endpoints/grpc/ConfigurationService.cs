...
public sealed class ConfigurationService : Configuration.ConfigurationBase
{
    ...
    public override async Task<GetConfigurationResponse> GetConfiguration(GetConfigurationRequest request, ServerCallContext context)
    {
        ...
        if (tenantId > 0)
        {
            List<Database.Models.UserSigningKey> signingKeys = await _cache.GetActiveSigningKeysForTenantAsync(tenantId, context.CancellationToken);
            foreach (Database.Models.UserSigningKey key in signingKeys)
            {
                response.SigningKeys.Add(new TrustedSigningKey
                {
                    KeyId = key.Id,
                    UserId = key.UserId,
                    PublicKey = Google.Protobuf.ByteString.FromBase64(key.PublicKey),
                });
            }
        }
    ...
    public override async Task<GetPendingCommandsResponse> GetPendingCommands(GetPendingCommandsRequest request, ServerCallContext context)
    {
        ...
        List<Database.Models.RemoteCommand> pendingCommands = await _cache.GetPendingCommandsForMachineAsync(machineId, tenantId, context.CancellationToken);

        GetPendingCommandsResponse response = new();
        List<string> deliveredIds = [];

        foreach (Database.Models.RemoteCommand cmd in pendingCommands)
        {
            AgentCommand agentCmd = new()
            {
                ...
                Signature = Google.Protobuf.ByteString.CopyFrom(Convert.FromBase64String(cmd.Signature)),
            ...
            {
                using System.Text.Json.JsonDocument paramsDoc = System.Text.Json.JsonDocument.Parse(cmd.Params);
                foreach (System.Text.Json.JsonProperty prop in paramsDoc.RootElement.EnumerateObject())
                {
    ...
    public override async Task<AcknowledgeCommandResponse> AcknowledgeCommand(AcknowledgeCommandRequest request, ServerCallContext context)
    {
        ...
        Database.Enums.RemoteCommandStatus status;
        if (result?.ResultType == ResultType.Rejected)
        {
            status = Database.Enums.RemoteCommandStatus.Rejected;
        }
        else if (result?.Success == true)
        {
            status = Database.Enums.RemoteCommandStatus.Executed;
        }
        else
        {
            status = Database.Enums.RemoteCommandStatus.Failed;
        }
