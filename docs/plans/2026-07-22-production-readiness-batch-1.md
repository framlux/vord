...
**Interfaces:**
- Produces: `RetentionClass` enum in the database project; `MachineTelemetry.RetentionClass` (smallint, part of partition routing); `IPartitionRepository.GetRetentionDaysByClassAsync(CancellationToken)` returning `IReadOnlyDictionary<RetentionClass, int>`.
- Class↔tier map: Free→Short, Pro→Medium, Team→Long. Retention days per class come from `TierFeatureLimits` per tier **and must take the MAX with any `TenantSubscriptionOverride` retention for tenants currently in that tier** — an override granting longer retention than the tier default must extend the class's physical window, never silently lose data. Write the SQL as: per class, `GREATEST(tier default, COALESCE(MAX(override retention for tenants of that tier), 0))`.
...
- [ ] **Step 4:** `PartitionManagementJob`: daily creation loops classes × days (leaf `CREATE TABLE IF NOT EXISTS "MachineTelemetry_{Class}_{yyyyMMdd}" PARTITION OF "MachineTelemetry_{Class}" FOR VALUES FROM … TO …`); drop path queries `GetRetentionDaysByClassAsync` and drops each class's leaves past its own cutoff. Keep the existing identifier-validation and bounded-lookback behavior. Delete `GetMaxRetentionDaysAsync` and its callers/tests (replaced, not kept alongside).

- [ ] **Step 5:** Ingest stamping: the envelope-processing path already holds the tenant's subscription (eligibility gate). Map tier→class with a small `internal static` (unit-tested: all four `SubscriptionTier` values — Free, Pro, Team, and whatever else the enum defines; unknown → Short, fail-safe cheapest). Stamp every inserted row.
...
- [ ] **Step 1:** Replace the two promises:
  - ~line 110 ("If you cancel your account, we will delete your data"): → "If you cancel your account, your data becomes inaccessible immediately and telemetry expires per your plan's retention window. To request full deletion of remaining account records, contact us at the address below and we will complete it within 30 days."
