...
public sealed class SqliteSqlDialectTests
{
    ...
    [Test]
    public async Task BuildUpsertSshSessions_SqlContainsPerTypeStaleDataGuard()
    {
        (string sql, string _) = _dialect.BuildUpsertSshSessions(null, """{"timestamp":"2026-01-01T00:00:00Z"}""");

        string guard = """WHERE @ts >= "MachineState"."SshSessionsAt" OR "MachineState"."SshSessionsAt" IS NULL""";
