...
/// <summary>
/// Intent-asserting tests for <see cref="DatabaseCache"/> input validation.
...
public class DatabaseCacheTests
{
    ...
    {
        await Assert.That(() =>
            new DatabaseCache(null!, Substitute.For<ILogger<DatabaseCache>>()))
    ...
    public async Task Constructor_NullLogger_ThrowsArgumentNullException()
    {
        ...
        await Assert.That(() =>
            new DatabaseCache(dbFactory.Context, null!))
            .Throws<ArgumentNullException>();
    }
    ...
    {
        using TestDatabaseFactory dbFactory = new();
        DatabaseCache cache = new(dbFactory.Context, new NullLogger<DatabaseCache>());
