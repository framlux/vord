...
public sealed class ViewOnlyEndpointTests
{
    ...
    [Test]
    public async Task MachineTelemetry_FilterByType_ReturnsOnlyMatchingType()
    {
        using FunctionalTestFactory factory = new();
        using DatabaseContext db = factory.CreateDbContext();

        int tenantId = await SeedTenantWithSubscription(db);
        long machineId = await SeedMachine(db, tenantId, "telemetry-host");

        // Seed telemetry of different types
        for (int i = 0; i < 3; i++)
        {
            ...
            await db.InsertWithInt64IdentityAsync(t);
        }
        ...
        HttpResponseMessage response = await client.GetAsync($"/api/v1/machines/{machineId}/telemetry?type=1");
    ...
    [Test]
    public async Task MachineTelemetryLatest_ReturnsOnePerType()
    {
        using FunctionalTestFactory factory = new();
        using DatabaseContext db = factory.CreateDbContext();

        int tenantId = await SeedTenantWithSubscription(db);
        long machineId = await SeedMachine(db, tenantId, "latest-host");

        // Seed multiple records per type
        for (int i = 0; i < 3; i++)
        {
            MachineTelemetry t = new()
            {
        ...
        HttpResponseMessage response = await client.GetAsync($"/api/v1/machines/{machineId}/telemetry/latest");
        ...
        // Verify exactly 2 telemetry entries (one per type).
        using JsonDocument doc = JsonDocument.Parse(body);
        JsonElement items = doc.RootElement.GetProperty("data");
        await Assert.That(items.GetArrayLength()).IsEqualTo(2);
        await Assert.That(body).Contains("\"telemetryType\":1");
        await Assert.That(body).Contains("\"telemetryType\":2");
    }
