...
public sealed class AnonymousEndpointTests
{
    ...
    [Test]
    public async Task ContactForm_NameContainsCarriageReturn_Returns400AndDoesNotInjectLogLine()
    {
        // The ContactForm endpoint logs only scalar field LENGTHS (not field values), but the
        // validator's CRLF rejection is the line of defense if that policy ever regresses. Hit
        // it directly with CR in the Name field and confirm the validator rejects it.
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
            AllowAutoRedirect = false,
        ...
        HttpResponseMessage response = await client.PostAsJsonAsync("/api/v1/contact", new
        {
            Name = "Mallory\rINFO: fake-log-line",
        ...
        await Assert.That(doc.RootElement.GetProperty("message").GetString()).IsEqualTo("Name must not contain newline characters");
    }

    [Test]
    public async Task ContactForm_NameContainsLineFeed_Returns400()
    {
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
        ...
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.BadRequest);
        string body = await response.Content.ReadAsStringAsync();
        await Assert.That(body).Contains("newline");
    }

    [Test]
    public async Task ContactForm_NameAtBoundary_Length120_Succeeds()
    {
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
        ...
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.OK);
        string body = await response.Content.ReadAsStringAsync();
        await Assert.That(body).Contains("Thank you");
    }

    [Test]
    public async Task ContactForm_NameJustOverBoundary_Length121_Returns400()
    {
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
        ...
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.BadRequest);
        string body = await response.Content.ReadAsStringAsync();
        await Assert.That(body).Contains("120 characters");
    }

    [Test]
    public async Task ContactForm_MessageJustOverBoundary_Length5001_Returns400()
    {
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
        ...
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.BadRequest);
        string body = await response.Content.ReadAsStringAsync();
        await Assert.That(body).Contains("5000 characters");
    }

    [Test]
    public async Task ContactForm_MalformedEmail_Returns400()
    {
        using FunctionalTestFactory factory = new();
        HttpClient client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
        ...
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.BadRequest);
        string body = await response.Content.ReadAsStringAsync();
        await Assert.That(body).Contains("valid email address");
    }
